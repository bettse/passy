...
asn_dec_rval_t SET_OF_decode_ber(
    ...
    case 0:
        ...
        rval = ber_check_tags(opt_codec_ctx, td, ctx, ptr, size, tag_mode, 1, &ctx->left, 0);
        if(rval.code != RC_OK) {
    ...
    case 1:
		 ...
		 */
			 ...
			 */
                    ...
                    ASN_DEBUG(
                        ...
                        td->name);
                    ASN_DEBUG("%s SET OF has tag %s", td->name, ber_tlv_tag_string(elm->tag));
                    RETURN(RC_FAIL);
                }
            }
		 ...
		 */
            rval = elm->type->op->ber_decoder(opt_codec_ctx, elm->type, &ctx->ptr, ptr, LEFT, 0);
            ASN_DEBUG(
...
static void SET_OF__encode_sorted_free(struct _el_buffer* el_buf, size_t count) {
    size_t i;
...
static struct _el_buffer* SET_OF__encode_sorted(
    ...
    encoded_els = (struct _el_buffer*)CALLOC(list->count, sizeof(encoded_els[0]));
    if(encoded_els == NULL) {
        return NULL;
    }
...
asn_enc_rval_t SET_OF_encode_der(
     ...
     */
    encoding_size = der_write_tags(td, computed_size, tag_mode, 1, tag, cb, app_key);
    if(encoding_size < 0) {
        ASN__ENCODE_FAILED;
    }
...
asn_enc_rval_t SET_OF_encode_xer(
    ...
    const asn_TYPE_member_t* elm = td->elements;
    const asn_anonymous_set_* list = _A_CSET_FROM_VOID(sptr);
    const char* mname = specs->as_XMLValueList ? 0 :
    ...
    for(i = 0; i < list->count; i++) {
        ...
        tmper = elm->type->op->xer_encoder(
            elm->type, memb_ptr, ilevel + (specs->as_XMLValueList != 2), flags, cb, app_key);
...
int SET_OF_print(
    ...
    /* Dump preamble */
    if(cb(td->name, strlen(td->name), app_key) < 0 || cb(" ::= {", 6, app_key) < 0) return -1;
    ...
    for(i = 0; i < list->count; i++) {
        ...
        ret = elm->type->op->print_struct(elm->type, memb_ptr, ilevel + 1, cb, app_key);
        if(ret) return ret;
    }
...
int SET_OF_constraint(
    ...
    if(!sptr) {
        ASN__CTFAIL(
            app_key, td, sptr, "%s: value not given (%s:%d)", td->name, __FILE__, __LINE__);
        return -1;
    }
...
asn_dec_rval_t SET_OF_decode_uper(
    ...
    if(ct && ct->effective_bits >= 0) {
        ...
        ASN_DEBUG(
            "Preparing to fetch %ld+%ld elements from %s", (long)nelems, ct->lower_bound, td->name);
    ...
    do {
        ...
        if(nelems < 0) {
            ...
            ASN_DEBUG(
                "Got to decode %" ASN_PRI_SSIZE " elements (eff %d)",
                nelems,
        ...
        for(i = 0; i < nelems; i++) {
            ...
            rv = elm->type->op->uper_decoder(
                opt_codec_ctx, elm->type, elm->encoding_constraints.per_constraints, &ptr, pd);
...
asn_enc_rval_t SET_OF_encode_uper(
    ...
    if(ct) {
        int not_in_root = (list->count < ct->lower_bound || list->count > ct->upper_bound);
        ASN_DEBUG(
            "lb %ld ub %ld %s",
            ct->lower_bound,
            ct->upper_bound,
    ...
    for(encoded_edx = 0; (ssize_t)encoded_edx < list->count;) {
        ...
        for(edx = encoded_edx; edx < encoded_edx + may_encode; edx++) {
            ...
            if(asn_put_many_bits(po, el->buf, (8 * el->length) - el->bits_unused) < 0) {
                break;
            }
        }

        if(need_eom && uper_put_length(po, 0, 0)) ASN__ENCODE_FAILED; /* End of Message length */
...
asn_random_fill_result_t SET_OF_random_fill(
    ...
    if(!constraints || !constraints->per_constraints) constraints = &td->encoding_constraints;
    ...
    for(; rnd_len > 0; rnd_len--) {
        ...
        asn_random_fill_result_t tmpres = elm->type->op->random_fill(
            elm->type,
            &ptr,
            &elm->encoding_constraints,
            (max_length > res_ok.length ? max_length - res_ok.length : 0) / rnd_len);
        switch(tmpres.code) {
        case ARFILL_OK:
